...
uniform float uSpeed; uniform int uStreakCount;
uniform float uStreakWidth,uStreakLength,uGlow,uDensity,uTwinkle,uZoom,uBgGlow,uOpacity;
...
/**
 * Lightfall — WebGL "light rain" background (adapted from React Bits).
 * Renders into a full-size container; the pointer light follows the cursor
 * over the container while the canvas itself is non-interactive.
 */
export default function Lightfall({
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      iMouse: { value: [gl.drawingBufferWidth * 0.5, gl.drawingBufferHeight * 0.6] },
      iTime: { value: 0 },
    ...
    const target: [number, number] = [gl.drawingBufferWidth * 0.5, gl.drawingBufferHeight * 0.6];
    ...
    if (mouseInteraction) container.addEventListener('pointermove', onMove);
    ...
    return () => {
      ...
      if (mouseInteraction) container.removeEventListener('pointermove', onMove);
      ro.disconnect();
    ...
    colorKey, backgroundColor, speed, streakCount, streakWidth, streakLength,
    glow, density, twinkle, zoom, backgroundGlow, opacity,
    mouseInteraction, mouseStrength, mouseRadius, mouseDampening, dpr,
  ]);

  return <div ref={containerRef} className={`lf-root ${className ?? ''}`} />;
}
